...
- **Bounded MPSC rings** with condvar backpressure — **never drop** on full
- **Preallocated frame pool** — no `malloc`/`free` on the hot path
...
| Topic | Choice |
|-------|--------|
| Queue | Mutex + condvar bounded ring (MPSC, cancel-safe cleanup) |
